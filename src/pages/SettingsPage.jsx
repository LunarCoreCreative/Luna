...
import {
    ...
    Cloud, Leaf, Heart, Coffee, Droplets, BookOpen, Gem, Star, Sparkle, Layers
} from 'lucide-react';
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-2">
                                {darkThemes.map(t => (
                                    <div
                                        ...
                                        className={`p-5 rounded-2xl border-2 transition-all cursor-pointer ${t.color} flex flex-col items-center gap-3 hover:scale-105 relative overflow-hidden ${theme === t.id ? 'ring-2 ring-white/30 border-white/40 opacity-100 scale-105' : 'opacity-60 hover:opacity-100'}`}
                                    >
                                        {theme === t.id && (
                                            <div className="absolute top-3 right-3 p-1.5 bg-white/90 rounded-full shadow-lg">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-2">
                                {lightThemes.map(t => (
                                    <div
                                        key={t.id}
                                        onClick={() => onThemeChange(t.id)}
                                        className={`p-5 rounded-2xl border-2 transition-all cursor-pointer ${t.color} flex flex-col items-center gap-3 hover:scale-105 relative overflow-hidden ${theme === t.id ? 'ring-2 ring-gray-400/50 border-gray-500/60 opacity-100 scale-105 shadow-lg' : 'opacity-70 hover:opacity-100'}`}
                                    >
                                        {theme === t.id && (
                                            <div className="absolute top-3 right-3 p-1.5 bg-gray-800/90 rounded-full shadow-lg">
